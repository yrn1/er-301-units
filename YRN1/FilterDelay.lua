...
function FilterDelay:loadStereoGraph()
    ...
    local constantOne = self:addObject("constantOne",app.Constant())
    constantOne:hardSet("Value",1.0)
    local constantZero = self:addObject("constantZero",app.Constant())
    constantZero:hardSet("Value",0.0)

    local eqL = self:addObject("eqL", libcore.Equalizer3())
    eqL:hardSet("Low Freq", 880.0)
    eqL:hardSet("High Freq", 880.0)
    connect(constantOne,"Out",eqL,"Low Gain")
    connect(constantOne,"Out",eqL,"Mid Gain")
    connect(constantZero,"Out",eqL,"High Gain")

    local eqR = self:addObject("eqR", libcore.Equalizer3())
    eqR:hardSet("Low Freq", 880.0)
    eqR:hardSet("High Freq", 880.0)
    connect(constantOne,"Out",eqR,"Low Gain")
    connect(constantOne,"Out",eqR,"Mid Gain")
    connect(constantZero,"Out",eqR,"High Gain")
